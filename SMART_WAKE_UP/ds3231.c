...
void DS3231_Read(uint8_t slav_add, uint8_t memadd, uint8_t *data, uint8_t length )
{
	...
	while(!(I2C1->ISR & I2C_ISR_TC) && timeout < TIMEOUT_MAX)	// Boucle d'attente de la condition TC (Transmission Complete)
	{ 
	...
	if (timeout >= (TIMEOUT_MAX - 10)) DS3231_HS = 1;
	else
	{
...
{
	uint8_t send_arr[length+1]; // Creation d'un tableau pour stocker les donnees a envoyer
	send_arr[0]=memadd; // Stockage de l'adresse memoire dans le tableau
	for (int i=1;(i<length+1);i++) send_arr[i]=*data++; // Stockage des donnees dans le tableau
	...
	while(!(I2C1->ISR & I2C_ISR_STOPF)) // Boucle d'attente de la condition STOPF (Stop detection flag)
	{
		if(I2C1->ISR & I2C_ISR_TXE) // Verification si le buffer de transmission est vide
		{
			I2C1->TXDR = send_arr[i] ; // Envoi des donnees
			i++;
		}
	}
